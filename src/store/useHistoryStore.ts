...
export const useHistoryStore = create<HistoryState>()(set=>({
    ...
    getLastSearch: async()=>{
        ...
        const lastSearchList = fetchLastSearch ? JSON.parse(fetchLastSearch) : null
        console.log("lastSearchList ", lastSearchList);
        
        const lastSearch:string|null = lastSearchList ? lastSearchList[0] : null
        return lastSearch
    },
    ...
    getLastTrack: async()=>{
        ...
        const lastTrack = fetchLastTrack ? JSON.parse(fetchLastTrack) as TrackData : null
        return lastTrack
    }
}))