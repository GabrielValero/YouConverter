...
export default function usePermissions(){
    ...
    const WriteStoragePermission = async () => {
        try {
            const verify = await PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE)
    ...
    //     try {
    //         const verify = await PermissionsAndroid.check(PermissionsAndroid.PERMISSIONS.POST_NOTIFICATIONS)
    //         console.log("por aqui paso ", verify);
    //         var granted = PermissionsAndroid.RESULTS.GRANTED
    //         if(!verify){
